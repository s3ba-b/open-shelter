...
| **M3 — Staff web application** | A Blazor staff-facing web app (`src/OpenShelter.Web`), Aspire-orchestrated and built on ServiceDefaults, authenticating through the M1 identity/tenant pipeline. The app shell (sign-in, tenant-aware navigation, layout) is established, and the Animals experience works end-to-end against the live Gateway API, strictly tenant-scoped. Later domain milestones extend this same app. |
