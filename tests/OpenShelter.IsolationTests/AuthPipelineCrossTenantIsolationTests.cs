...
using Xunit;
using LoginRequest = identity::OpenShelter.Identity.Api.Auth.LoginRequest;
using LoginResponse = identity::OpenShelter.Identity.Api.Auth.LoginResponse;
...
public sealed class AuthPipelineCrossTenantIsolationTests : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        // keeps these globals from racing with the M0 test's openshelterdb value.
        Environment.SetEnvironmentVariable("ConnectionStrings__identitydb", _identityDb.GetConnectionString());
        Environment.SetEnvironmentVariable("ConnectionStrings__openshelterdb", _animalsDb.GetConnectionString());
